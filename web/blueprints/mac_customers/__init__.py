...
def index():
    ...
    stats_query = (
        "SELECT "
        "COUNT(*) as total, "
        "COALESCE(SUM(CASE WHEN (SELECT COUNT(*) FROM active_sessions s WHERE s.username = c.username) > 0 THEN 1 ELSE 0 END), 0) as online_count, "
        "COALESCE(SUM(CASE WHEN (SELECT COUNT(*) FROM active_sessions s WHERE s.username = c.username) = 0 AND c.status = 'active' THEN 1 ELSE 0 END), 0) as offline_count, "
        "COALESCE(SUM(CASE WHEN c.status = 'isolir' THEN 1 ELSE 0 END), 0) as isolir_count, "
        "COALESCE(SUM(COALESCE(p.price, 0)), 0) as total_revenue "
        "FROM customers c "
        "LEFT JOIN profiles p ON c.profile_id = p.id "
        "WHERE (c.mac_address IS NOT NULL AND c.mac_address != '')"
    )
